...
function getFlagEmoji(countryCode) {
  ...
  const flagFallbacks = {
    ...
    "XT": "🇪🇺", // 欧州全域 (Chess.comコード)
  };

  // フォールバック辞書にある場合はそれを使用
  if (flagFallbacks[countryCode.toUpperCase()]) {
    return flagFallbacks[countryCode.toUpperCase()];
  }
...
const countryCoordinates = {
  ...
  "XB": { lat: 50.8503, lng: 4.3517, name: "ベルギー (Chess.com)" },
  "XC": { lat: 55.6761, lng: 12.5683, name: "デンマーク (Chess.com)" },
  "XT": { lat: 54.5260, lng: 15.2551, name: "欧州全域" },
...
async function fetchChessData(period = 3) {
  try {
    ...
    if (period === "selected") {
      ...
      // 選択された期間をカンマ区切りで送信
      const periodsParam = selectedPeriods.map((p) =>
        `${p.year}/${String(p.month).padStart(2, "0")}`
      ).join(",");
      apiUrl = `/api/chess-data?period=selected&periods=${
        encodeURIComponent(periodsParam)
      }`;
    } else {
...
function displayCountryList(countries) {
  ...
  // 国を対戦数でソート
  const sortedCountries = Array.from(countries.entries()).sort((a, b) =>
    b[1] - a[1]
  );
  ...
  sortedCountries.forEach(([countryCode, count]) => {
    ...
    li.innerHTML = `
            ...
            <div class="country-info">
                <div class="country-name">${countryInfo.name} (${countryCode})</div>
...
function getSelectedPeriods() {
  ...
  const checkboxes = document.querySelectorAll(
    '#periodsList input[type="checkbox"]:checked',
  );
...
document.getElementById("fetchButton").addEventListener("click", async () => {
  ...
  try {
    // ローディング状態の更新
    const periodText = selectedPeriods.map((p) =>
      `${p.year}年${String(p.month).padStart(2, "0")}月`
    ).join(", ");
    updateLoadingStatus(
      `Chess.comからデータを取得中... 対象期間: ${periodText} (${selectedPeriods.length}期間)`,
    );
    ...
    // 地図処理中のローディング状態更新
    updateLoadingStatus(
      `地図データを処理中... 対戦国数: ${countries.size}カ国, 総対戦数: ${games.length}ゲーム`,
    );

    // 新規データ取得後、既存データと合わせて地図を更新
    updateLoadingStatus(
      `既存データと合併中... 新規取得データ: ${countries.size}カ国, 全データを地図に反映中...`,
    );
  ...
  try {
    ...
    if (response.ok && data.allPeriods && data.allPeriods.length > 0) {
      ...
      const existingCheckboxes = document.querySelectorAll(
        '#periodsList input[type="checkbox"]',
      );
...
function setupSelectAllFunctionality() {
  ...
  const periodCheckboxes = document.querySelectorAll(
    '#periodsList input[type="checkbox"]',
  );
...
async function loadExistingDataAndDisplay() {
  try {
    ...
    if (data.countries && data.countries.length > 0) {
      ...
      console.log(
        `既存データを読み込みました: ${countries.size}カ国, ${
          Array.from(countries.values()).reduce((a, b) => a + b, 0)
        }ゲーム`,
      );
    }
  } catch (error) {
