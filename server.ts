...
class DiskStorage {
  ...
  async get<T>(key: string): Promise<T | null> {
    try {
      ...
      // データの有効期限チェック
      const now = Date.now();
      const maxAge = entry.isCurrentMonth
        ? 30 * 60 * 1000
        : 24 * 60 * 60 * 1000; // 当月は30分、それ以外は24時間
...
async function getAllAvailablePeriods(
  username: string,
): Promise<Array<{ period: string; year: number; month: number }>> {
...
async function fetchChessData(
  username: string,
  monthsBack: number | string | { type: string; periods: string[] } = 3,
) {
  ...
  try {
    ...
    if (monthsBack === "all") {
      ...
      typeof monthsBack === "object" && monthsBack.type === "selected"
    ) {
      // 選択された期間のみ
    ...
    console.log(
      `Starting efficient data fetch for: ${username}, ${logMessage}`,
    );
    ...
    for (const archiveUrl of recentArchives) {
      try {
        ...
        const urlMatch = archiveUrl.match(
          /\/player\/([^\/]+)\/games\/(\d{4})\/(\d{2})$/,
        );
        if (!urlMatch) {
          console.error(`Invalid archive URL format: ${archiveUrl}`);
          continue;
        }
        const [, _urlUsername, year, month] = urlMatch;
        ...
        if (!gamesData) {
          console.log(
            `Fetching ${
              isCurrentMonth ? "current month" : "archived"
            } data: ${archiveUrl}`,
          );
          ...
          if (gamesResponse.ok) {
            ...
            console.log(
              `Saved archive data: ${gamesData.games?.length || 0} games`,
            );
          }
        } else {
...
async function fetchPlayerDetails(username: string) {
  ...
  const savedData = await storage.get(dataKey);
  if (savedData) {
    return savedData;
  }
...
// 対戦相手の国を抽出
async function extractOpponentCountries(games: unknown[], username: string) {
  ...
  for (const opponentUsername of opponents) {
    processedCount++;
    console.log(
      `Fetching details for opponent ${processedCount}/${opponents.size}: ${opponentUsername}`,
    );
    ...
    if (playerDetails && playerDetails.country) {
      ...
      if (countryCode && countryCode.length === 2) {
        ...
        console.log(
          `Added ${gameCount} games for ${opponentUsername} from ${countryCode}`,
        );
      }
    } else {
...
// 特定ユーザーの既存データを読み込む関数
async function loadExistingDataForUser(
  ...
  try {
    ...
    try {
      for await (const dirEntry of Deno.readDir(archivesDir)) {
        if (dirEntry.isFile && dirEntry.name.endsWith(".json")) {
          const match = dirEntry.name.match(/^(.+)_\d{4}_\d{2}\.json$/);
          if (
            match && match[1].toLowerCase() === targetUsername.toLowerCase()
          ) {
            try {
              ...
              console.error(
                `Error reading archive file ${dirEntry.name}:`,
                error,
              );
            }
          }
    ...
    for (const opponentUsername of opponents) {
      try {
        ...
        console.error(
          `Error loading player data for ${opponentUsername}:`,
          error,
        );
      }
    }
...
async function handler(req: Request): Promise<Response> {
  ...
  if (url.pathname === "/api/chess-data") {
    ...
    if (periodParam === "all") {
      ...
      if (!periodsParam) {
        return new Response(
          JSON.stringify({ error: "Selected periods are required" }),
          {
            status: 400,
    ...
    if (!username) {
      return new Response(
        JSON.stringify({ error: "Username not set at server startup" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        },
...
async function startServer() {
  ...
  try {
    ...
    if (userCountries.size > 0) {
      const totalGames = Array.from(userCountries.values()).reduce(
        (a, b) => a + b,
        0,
      );
      console.log(`\n${username} の既存データ:`);
      ...
      // 上位10カ国のデータを表示
      console.log("\n=== 対戦数上位10カ国 ===");
      const sortedCountries = Array.from(userCountries.entries()).sort((a, b) =>
        b[1] - a[1]
      );
